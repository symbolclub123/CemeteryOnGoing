...
function isDeceasedPersonExists($conn, $lastName, $firstName, $middleName) {
    // Query the database to check if a person with the same full name exists
    $query = "SELECT COUNT(*) AS count FROM deceased_records WHERE last_name = '$lastName' AND first_name = '$firstName' AND middle_name = '$middleName'";
    $result = mysqli_query($conn, $query);
...
function insertRecord($conn, $block, $lot, $lastName, $firstName, $middleName, $barangay, $cityMunicipality, $province, $deceasedSex, $religion, $nationality, $civilStatus, $occupation, $dateOfDeath, $causeOfDeath, $placeOfDeath, $informantId, $relation, $deathRef) {
    // Generate year, month, and sequence
    ...
    if (!validateDateOfDeath($dateOfDeath)) {
    ...
    } else if (isDeceasedPersonExists($conn, $lastName, $firstName, $middleName)) {
        $_SESSION["error"] = "The full name already exists.";
    } else {
        // Check the latest sequence number for the current year and month
        $query = "SELECT COALESCE(MAX(sequence), 0) AS last_sequence FROM deceased_records WHERE year = '$year' AND month = '$month'";
        $result = mysqli_query($conn, $query);
        ...
        if ($result) {
            ...
            // Insert the data with the custom primary key
            $insertDataQuery = "INSERT INTO deceased_records (id, year, month, sequence, block, lots, last_name, first_name, middle_name, barangay, city_municipality, province, deceased_sex, religion, nationality, civil_status, occupation, date_of_death, cause_of_death, place_of_death, informant_id, relation, status, death_reference)
                                VALUES ('$customPrimaryKey', '$year', '$month', $newSequence, '$block', '$lot', '$escapedLastName', '$escapedFirstName', '$escapedMiddleName', '$escapedBarangay', '$escapedCityMunicipality', '$escapedProvince', '$escapedDeceasedSex', '$escapedReligion', '$escapedNationality', '$escapedCivilStatus', '$escapedOccupation', '$escapedDateOfDeath', '$escapedCauseOfDeath', '$escapedPlaceOfDeath', '$escapedInformantId', '$escapedRelation', 'OCCUPIED', '$escapedDeathRef')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Call the insertRecord function with the form data
    insertRecord($conn, $block, $lot, $lastName, $firstName, $middleName, $barangay, $cityMunicipality, $province, $deceasedSex, $religion, $nationality, $civilStatus, $occupation, $dateOfDeath, $causeOfDeath, $placeOfDeath, $informantId, $relation, $deathRef);
}
...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-30 w-64 h-screen pt-20 transition-transform transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0">
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 rounded-lg mt-14">
            <h1 class="text-3xl text-primary font-semibold mb-4 ">Add Deceased Person to (Block <?= $block; ?> & lot <?= $lot; ?>)</h1>
